...
def get_amazon_data_dict(dataset_path):
    ...
    print('========== Start Amazon ==========')
    train, val, test = generate_seq_feature(data=data, user_col="user_id",
                                            item_col="video_id", time_col='time',
                                            item_attribute_cols=["cate_id","music_type"],
                                            user_attribute_cols=["active_degree_id"]) # 这里只用考虑用户行为来输出的行为序列特征
    print('INFO: Now, the dataframe named: ', train.columns)
    n_users, n_items, n_cates, n_degree = data["user_id"].max(), data["video_id"].max(),data["cate_id"].max(),data["active_degree_id"].max()
    n_mtype = 6 #自定义视频音乐类型
    ...
    features = [SparseFeature("target_item_id", vocab_size=n_items+1, embed_dim=8),
                ...
                SparseFeature("target_music_type", vocab_size=n_mtype+1, embed_dim=8),
                SparseFeature("target_active_degree_id", vocab_size=n_mtype + 1, embed_dim=8)]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_path', default="./data/amazon-electronics/data03.csv")
