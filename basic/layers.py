...
# 这里activation_layer是激活函数的内容 feature是特征框架

# 接受logits作为输入，并在分类任务中应用sigmoid激活，或者在回归任务中直接输出logits
...
# 该模块用于为不同类型的特征创建嵌入表。接受特征值，并为每种类型的特征创建嵌入
    ...
    def __init__(self, input_dim, cin_size, split_half=True):
        ...
        for i in range(self.num_layers):
            ...
            self.conv_layers.append(
                torch.nn.Conv1d(input_dim * prev_dim, cross_layer_size, 1, stride=1, dilation=1, bias=True))
...
class BiLinearInteractionLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.bilinear_type == "field_all":
            ...
            bilinear_list = [self.bilinear_layer[i](feature_emb[i]) * feature_emb[j] for i, j in
                             combinations(range(len(feature_emb)), 2)]
        elif self.bilinear_type == "field_interaction":
...
class CEN(nn.Module):
    ...
    def __init__(self, embed_dim, num_field_crosses, reduction_ratio):
        ...
        self.mlp_att = MLP(num_field_crosses, dims=[num_field_crosses // reduction_ratio, num_field_crosses],
                           output_layer=False, activation="relu")
